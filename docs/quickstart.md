# How to Use HALvesting

Building the dataset is a three step process: data fetching from HAL, data merging and data enriching.

It's easier to modify the files `scripts/fetch_data.sh`, `scripts/merge_data.sh` and `scripts/enrich_data.sh` at need and launch them. However one can launch directly the Python scripts with the correct arguments as we will see below.
